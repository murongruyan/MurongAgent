...
internal object GitHubAuthFlow {
    fun buildAuthorizationUrl(config: ProviderConfig, clientState: String): String {
        val baseUrl = config.getMurongBackendAuthApiUrl().trim()
        ...
        return buildString {
            ...
            append(encodeQueryValue(config.getMurongGitHubRedirectUri()))
            append("&client_state=")
    ...
    fun isGitHubOAuthCallback(rawCallbackUri: String): Boolean {
        ...
        val isSupportedScheme = callbackUri.scheme.equals("murongagent", ignoreCase = true)
