...
func TestCheckDesktopUpdatePrefersManifestVersion(t *testing.T) {
	client := &http.Client{Transport: roundTripFunc(func(request *http.Request) (*http.Response, error) {
		...
		case strings.HasSuffix(request.URL.Path, "/releases/latest"):
			body = `{
				"html_url":"https://github.com/murongruyan/MurongAgent/releases/tag/murong-suite-v1.41",
				"tag_name":"murong-suite-v1.41",
				"body":"# MurongAgent 1.41",
				"published_at":"2026-07-28T00:00:00Z",
				"assets":[
					{"name":"release-manifest.json","browser_download_url":"https://github.com/murongruyan/MurongAgent/releases/download/murong-suite-v1.41/release-manifest.json"},
					{"name":"murong-desktop-agent-windows-amd64.exe","browser_download_url":"https://github.com/murongruyan/MurongAgent/releases/download/murong-suite-v1.41/murong-desktop-agent-windows-amd64.exe"}
				]
			}`
		case strings.HasSuffix(request.URL.Path, "/release-manifest.json"):
			body = `{"versions":{"desktopAgent":{"name":"1.41"}}}`
		default:
	...
	if info.LatestVersion != "1.41" || !info.UpdateAvailable {
