...
public final class TerminalView extends View {
    private static Typeface sResolvedDefaultTypeface;
    ...
    public TerminalView(Context context, AttributeSet attributes) { // NO_UCD (unused code)
        ...
        mGestureRecognizer = new GestureAndScaleRecognizer(context, new GestureAndScaleRecognizer.Listener() {
            ...
            public void onScaleEnd(float scale) {
                if (mEmulator == null || isSelectingText()) return;
                mScaleFactor = mClient.onScaleEnd(mScaleFactor);
            }
    ...
    public void setTextSize(int textSize) {
        mRenderer = new TerminalRenderer(textSize, mRenderer == null ? resolveDefaultTypeface() : mRenderer.mTypeface);
        updateSize();
    }

    public void setTypeface(Typeface newTypeface) {
        int textSize = mRenderer == null ? 16 : mRenderer.mTextSize;
        mRenderer = new TerminalRenderer(textSize, newTypeface != null ? newTypeface : resolveDefaultTypeface());
        updateSize();
        invalidate();
    }

    private Typeface resolveDefaultTypeface() {
        if (sResolvedDefaultTypeface != null) return sResolvedDefaultTypeface;

        Typeface fallback = Typeface.create("sans-serif-monospace", Typeface.NORMAL);
        if (fallback == null) fallback = Typeface.MONOSPACE;

        sResolvedDefaultTypeface = fallback;
        return fallback;
    }

    private boolean isEffectivelyMonospace(Typeface typeface) {
        Paint probe = new Paint();
        probe.setTypeface(typeface);
        probe.setTextSize(64f);
        float widthI = probe.measureText("i");
        float widthL = probe.measureText("l");
        float widthR = probe.measureText("r");
        float widthT = probe.measureText("t");
        float widthM = probe.measureText("M");
        float widthW = probe.measureText("W");
        float maxWidth = Math.max(Math.max(Math.max(widthI, widthL), Math.max(widthR, widthT)), Math.max(widthM, widthW));
        float minWidth = Math.min(Math.min(Math.min(widthI, widthL), Math.min(widthR, widthT)), Math.min(widthM, widthW));
        return maxWidth > 0f && (maxWidth - minWidth) <= 0.01f;
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        updateParentTouchInterception(event);
    ...
    private void updateParentTouchInterception(MotionEvent event) {
