...
在 `设置 → 看图与图片生成` 中，可为图片理解单独选择视觉模型。开启独立看图路由后，图片先由该模型转成可复用的中文观察摘要，后续聊天不会重复上传同一张历史图片给主模型。

图片生成使用 OpenAI Images 兼容接口 `POST /v1/images/generations`，支持局部预览、取消、重试和保存图片。`gpt-image-2` 支持 `2048x2048`、`2048x1152`、`3840x2160` 和 `2160x3840` 等原生尺寸；官方限制是单边不超过 3840、两边为 16 的倍数、总像素不超过 8294400、宽高比不超过 3:1。原生 4K 保证的是输出尺寸，不保证文字、细节或构图绝对正确。非 GPT Image 模型或需要额外放大时，生成结果卡片可调用 Replicate `nightmareai/real-esrgan` 进行 2–4 倍真实超分；只有长边至少 `3840px` 才会显示“真实 4K 超分完成”，并支持进度、取消、重试和保存。

自定义视觉 / 生图 / 真实 4K 超分 API Key 在 Android 使用 Keystore、在 Windows 使用 DPAPI 保存。普通备份、导出、聊天记录和日志不包含这些 Key；跨端同步时也必须同时明确勾选设置和供应商凭据。
