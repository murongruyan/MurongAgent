...
- 累积的 Android/Desktop 源码已以提交 `d7dcfea` 推送到 `origin/main`。Android GitHub 原生构建 `29733748341` 成功产出 Release APK；首轮桌面构建 `29733748434` 则暴露出两个真实跨平台缺陷：macOS 会把 `/var/...` 临时目录解析为 `/private/var/...`，旧比较逻辑会把同一项目误判为不同目录；Wails 固定生成 `Murong Desktop Agent.app`，旧打包步骤却按目标文件名寻找不存在的 `.app`。此外，PowerShell 多命令步骤没有在每次 `go test`/`go vet` 后检查退出码，导致前置测试失败可能被后续成功命令覆盖成假绿。
- 已把工作区身份统一为解析符号链接后的规范绝对路径，并覆盖最近项目、会话绑定、项目配置键、审计监听和持久设置；补充目录别名回归，并同时设置 Unix `HOME` 修正工作流导出测试。桌面工作流现在对每个原生命令显式 fail-fast，macOS 从 Wails 实际 `.app` 目录打出目标命名 ZIP。本地 Desktop Agent `go test -count=1 ./...`、`go vet ./...`、固定 `actionlint v1.7.12` 与 `git diff --check` 已通过；下一轮六目标原生矩阵必须连同完整性门禁全部成功，才接受正式桌面产物。
- 修复提交 `0d03e6b` 的首轮严格矩阵证明 fail-fast 已生效，并在 Windows x64 与两种 macOS runner 一致拦住一个遗漏的备份测试：跨系统恢复确实保留了同一项目和本机凭据，但断言仍要求规范化前后的路径字符串逐字相等。该断言已改为使用生产代码同一套工作区身份比较，凭据仍保持精确值校验；这不是放宽恢复规则，而是允许 macOS `/var`/`/private/var`、Windows runner 路径别名等指向同一目录的合法表示。
...
- 正式运行 `29739234378` 再次通过全部构建和完整性门禁，并成功发布 `murong-suite-v1.30`：10 个包加 `SHA256SUMS.txt`、`release-manifest.json` 共 12 个 Release 资产。第一次移动后端同步先上传 29,368,722 字节主 APK 1.30，随后在旧 PHP/Nginx 链路上传约 120 MB 扩展 1.10 时收到 `413 Request Entity Too Large`；Go 后端上线后仅重跑失败 Job，attempt 2 的 `Sync Mobile Packages To Murong Backend`（Job `88451409366`）在 55 秒内成功上传并发布两个 APK，无需重建桌面包。线上动态版本现为主程序 1.30/`26071902`、扩展 1.10/`26071720`，两份 APK 的完整下载字节数和实算 SHA-256 均与发布元数据一致。

### 2026-07-20 P5b.17（完整 Go 后端与一键原子部署，上线验收中）

- 用户决定不再继续为旧 PHP/宝塔链路单独调大上传限制，而是先把 `murongagent-backend` 改成与 `murongdiaodu-backend-go` 同类的 Go 服务和 GitHub 一键部署方案。GitNexus 重新索引两个后端后，现有 Go `Load`/`NewHandler` 只有主进程一个直接调用者，发布、OAuth、用量和管理统计各自只被对应 PHP API 入口使用，影响均为 LOW；因此保留既有 `.php` URL 作为客户端兼容路径，以 Nginx 一次切流，完整验收后删除旧 PHP 运行代码，不改 Android、桌面或管理端地址。
...
- 后端最终修复已推送到 `murongagent-backend/main` 的 `20d6d28`，主仓库统一引导与 Token 摘要配置已推送到 `MurongAgent/main` 的 `a67a6c3`。后端工作流 `29770869659` 通过 Linux `-race`、`go vet`、ShellCheck、Python Nginx 补丁器、PHP 契约和 amd64/arm64 包门禁，并把 `server-latest` 更新到相同源码版本；本地最终包也在 WSL 内逐文件复算 SHA-256、确认 23 个文件和 LF 脚本。
- 首次生产安装在 CentOS 7/systemd 219 上完成：专用 `murongagent` 账号、两个 systemd 服务、Nginx API/下载/Relay 路由、静态官网和 DeployHook 均生效。安装器只在原子窗口临时移除宝塔对精确 systemd 目录的 immutable 标志，退出时恢复；本地/公网健康均返回 `murongagent-backend-go`，DeployHook 返回 `atomic-go-v1`，本机直连、强制本机 TLS vhost 和公网 WSS 三次密文往返均通过。针对 Nginx graceful reload 的旧 worker 短暂 404，安装器现会校验 Go 身份并有限重试，不再因单次旧 worker 响应误回滚。
- 手动触发后端工作流 `29772646273` 并开启 `deploy_to_server=true`，测试、双架构构建、`server-latest` 更新和“部署到正式服务器”全部成功，证明后续可以只点 GitHub 工作流完成原子更新，不再上传文件或进入宝塔。GitHub Actions 临时 Artifact 存储额度已满产生注解，但滚动 Release 和正式部署步骤均成功，不影响服务器更新链路。
- 旧 PHP 运行后端已通过 `2d36bf4` 从源码删除，`api/classes/config`、旧手工 SQL 和 PHP OAuth 契约共清理 2,966 行；`murongagent.rl1.cc` 只保留静态官网，`.php` URL 全由 Go 路由处理。安装器提交 `ddb353f` 把生产遗留目录纳入恢复快照，在配置与 APK 迁移完成后清理，后续服务、Nginx 或 WSS 验收失败仍会原样回滚。Push 工作流 `29773608779`、`29773947918` 和正式一键部署 `29774085905` 均通过 Linux `-race`、ShellCheck、双架构打包和 Release 更新；最后一轮“部署到正式服务器”成功。
...
- 生产验收已确认新首页、下载页和 GitHub 页面内容，OAuth start 返回 GitHub 302 且包含 Client ID、本站 callback、`read:user repo` 和签名 state，恶意外域回跳返回 400；合法流程在 Chrome 中自动回到 `/admin/`，但管理后台最终账号/会话界面仍待一次人工目视确认。Usage 有效 heartbeat 返回 200、缺字段返回 400；Android 源码使用的三个 `.php` API 常量、两个 artifact key 和官方 WSS 地址与 Go 路由一致。主 APK 29,368,722 字节、扩展 APK 120,217,756 字节的跟踪 302、直接下载、Range 206、MIME、版本码和完整 SHA-256 均通过。
