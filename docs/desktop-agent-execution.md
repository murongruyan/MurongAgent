...
- 正式运行 `29739234378` 再次通过全部构建和完整性门禁，并成功发布 `murong-suite-v1.30`：10 个包加 `SHA256SUMS.txt`、`release-manifest.json` 共 12 个 Release 资产。可选后端同步在完整上传 29,368,722 字节主 APK 后收到 Nginx `413 Request Entity Too Large`；旧 15 MB 包可传而 1.30/1.10 分别约 29 MB/120 MB，因此这是线上站点请求体上限，不是构建或签名失败。线上仍保持 1.27/1.9，修复要求站点 `client_max_body_size 256m`，PHP 8.2 `upload_max_filesize=256M`、`post_max_size=260M`，生效后只需重跑移动同步，无需重建桌面包。工作流同步步骤增加 150 分钟总上限、慢速停滞检测、明确文件大小日志和 `Expect: 100-continue`，避免下次传完整个文件后才暴露 413。
